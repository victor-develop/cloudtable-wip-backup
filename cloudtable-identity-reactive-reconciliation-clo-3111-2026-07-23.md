# CLO-3111 CloudTable Identity/Reactive Reconciliation

Date: 2026-07-23 HKT
Goal: Identity and reactive data workflows
Issue: [CLO-3111](/CLO/issues/CLO-3111)
Parent: [CLO-3110](/CLO/issues/CLO-3110) CloudTable 15-minute project driver

## Disposition

No fresh bounded, non-duplicative implementation slice is warranted after
[CLO-3109](/CLO/issues/CLO-3109).

[CLO-3110](/CLO/issues/CLO-3110) is blocked only by this reconciliation child.
...
[CLO-3105](/CLO/issues/CLO-3105), and [CLO-3103](/CLO/issues/CLO-3103).

## Capability Reconciliation

- Google login remains represented by the Google auth login/callback flow,
  OAuth exchange, canonical identity linking, collision rejection, and signed
  auth session hydration in `src/runtime/worker.ts`.
- Invited users across organizations and organization workspaces remain
  ...
  and workspace membership, active workspace selection, tenant listing, member
  listing, and workspace switching in `src/runtime/worker.ts`, with persistence
  contracts in `src/core/persistence/types.ts`, D1 schema support in
...
- Declarative cross-table sync remains represented by the `direct_sync` recipe,
  routed workflow dependency metadata in
  `src/runtime/workflow-dependency-index.ts`, sync maintenance execution in
  `src/runtime/aggregate-maintenance.ts`, and agent-tool workflow maintenance
  request/read surfaces in `src/core/agent-tools`.
- Rolling computed columns remain represented by the `grouped_rollup` recipe,
  ...
  registry-backed aggregate operations, and workflow-service scoped maintenance
  writes.
- Extensible aggregate operations currently include `count_records`,
  `sum_numbers`, `max_number`, `min_number`, and `average_numbers` in
  `src/core/aggregates/registry.ts`, with validation and runtime coverage for
  these operation ids.
- Reactive computations support batch initialize/backfill through
  `workflow_backfill_jobs`, queue continuation, dependency operation reads, and
  manual aggregate/lookup/sync maintenance requesters across
...
- [CLO-3110](/CLO/issues/CLO-3110) is blocked by
  [CLO-3111](/CLO/issues/CLO-3111) and can resume when this checkpoint closes.
...
- [CLO-2930](/CLO/issues/CLO-2930) remains a separate aggregate remediation lane
  with its own blocker/recovery path; it is not a new identity/reactive backend
  implementation gap to duplicate from this checkpoint.
...
Close [CLO-3111](/CLO/issues/CLO-3111) as `done`. Do not create another
identity/reactive backend implementation issue from this checkpoint. Let
[CLO-3110](/CLO/issues/CLO-3110) resume through its first-class blocker link
