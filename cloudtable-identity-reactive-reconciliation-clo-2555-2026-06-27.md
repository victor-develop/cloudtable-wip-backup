# CLO-2555 CloudTable Identity/Reactive Reconciliation

Date: 2026-06-27
Goal: Identity and reactive data workflows
Issue: [CLO-2555](/CLO/issues/CLO-2555)
Parent: [CLO-2554](/CLO/issues/CLO-2554) CloudTable 15-minute project driver

## Disposition

No fresh bounded, non-duplicative implementation slice exists after
[CLO-2550](/CLO/issues/CLO-2550).

The active issue state still points to the same live lane:
[CLO-2321](/CLO/issues/CLO-2321) remains `in_review` for production
OAuth/deployment hardening, while its implementation and QA children
[CLO-2322](/CLO/issues/CLO-2322) and [CLO-2323](/CLO/issues/CLO-2323) are
`done`. The current code already covers the identity and reactive workflow
capability surface required by [CLO-2554](/CLO/issues/CLO-2554), so creating
another implementation child from this checkpoint would duplicate existing
identity, workflow-authoring, reactive-maintenance, or OAuth hardening work.

## Evidence

- Active goal remains `d6d73d1d-9a36-4411-a8b3-508859276d79`, `Identity and
  reactive data workflows`.
- [CLO-2554](/CLO/issues/CLO-2554) is blocked by this child,
  [CLO-2555](/CLO/issues/CLO-2555), and should resume after this child closes.
- Current issue state checked during this heartbeat:
  - [CLO-2550](/CLO/issues/CLO-2550), [CLO-2548](/CLO/issues/CLO-2548), and
    [CLO-2546](/CLO/issues/CLO-2546) are `done`.
  - [CLO-2321](/CLO/issues/CLO-2321) is still `in_review`.
  - [CLO-2322](/CLO/issues/CLO-2322) and [CLO-2323](/CLO/issues/CLO-2323) are
    `done`.
- Current source inspection still shows:
  - Google OAuth login/callback, tenant listing, invitation issuance/acceptance,
    workspace membership checks, active workspace selection, and session
    handling in `src/runtime/worker.ts`.
  - Invitation persistence and workspace/organization invitation context in
    `src/core/persistence/cloudtable-d1-repository.ts`.
  - Declarative `direct_sync` and `grouped_rollup` recipe metadata in
  ...
  - Manual aggregate and sync maintenance/backfill/recompute ingress in
    `src/runtime/worker.ts`, with runtime handling in
    `src/runtime/workflow-runtime.ts` and coordinator-owned writes in
...
Close [CLO-2555](/CLO/issues/CLO-2555) as done. Do not create a new
identity/reactive implementation child from this checkpoint. Let
[CLO-2554](/CLO/issues/CLO-2554) resume with this evidence, and keep
