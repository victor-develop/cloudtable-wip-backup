# CLO-3008 CloudTable Identity/Reactive Reconciliation

Date: 2026-07-22 HKT / 2026-07-21 UTC
Goal: Identity and reactive data workflows
Issue: [CLO-3008](/CLO/issues/CLO-3008)
Parent: [CLO-3007](/CLO/issues/CLO-3007) CloudTable 15-minute project driver

## Disposition

No fresh bounded, non-duplicative implementation slice is warranted for
[CLO-3007](/CLO/issues/CLO-3007).

The current repository and active issue tree still show the identity/reactive
workflow capability surface as already represented: Google login and callback
handling, invited users across organizations/workspaces, session workspace
selection, declarative direct cross-table sync, grouped rolling computed
columns, value-match resolver support, extensible aggregate operations, explicit
workflow service identity, and batch initialize/backfill/recompute maintenance.

Current movement should stay on the existing production hostname/Google OAuth
callback readiness lane rather than opening another backend identity/reactive
implementation child. Opening a new child from this checkpoint would duplicate
the existing runtime, authoring, maintenance, and OAuth/deployment lanes.

## Evidence

- [CLO-3007](/CLO/issues/CLO-3007) is blocked only by this reconciliation child,
  [CLO-3008](/CLO/issues/CLO-3008).
- The active goal remains `d6d73d1d-9a36-4411-a8b3-508859276d79`, `Identity and
  reactive data workflows`.
- Active goal inspection found [CLO-2647](/CLO/issues/CLO-2647) still
  `in_review` for production domain / Google OAuth callback origin readiness,
  with related hostname/OAuth rollout tasks already present. That is the
  standing non-runtime continuation path.
- Current source inspection shows:
  ...
  - Regression contracts for invitation-backed sync/rollups, invited-member
    read parity, workflow service identity, batch backfill, average/max
    rollups, and value-match max rollups in
...
Close [CLO-3008](/CLO/issues/CLO-3008) as `done`. Do not create a new
identity/reactive implementation child from this checkpoint. Let
[CLO-3007](/CLO/issues/CLO-3007) resume through its first-class blocker link
