# CLO-2225 CloudTable Identity/Reactive Reconciliation

Date: 2026-06-22
Goal: Identity and reactive data workflows
Issue: CLO-2225
Parent: CLO-2224 CloudTable 15-minute project driver
Prior checkpoint: CLO-2223
...
The active goal tree and current repository state still match the `CLO-2223`
checkpoint. The live CloudTable capability goal remains product-grade
multi-tenant identity plus declarative reactive workflows: Google login,
organization/workspace invitations, direct cross-table sync, grouped rolling
computed columns, extensible aggregate operations, and batch initialize/backfill.

The only active goal issue in this slice is the parent driver `CLO-2224` blocked
by this reconciliation child. There is no separate active implementation issue
to duplicate, and the current tracked source/test delta is already the bounded
workflow recipe authoring/create/publish lane for `direct_sync` and
`grouped_rollup`.

Creating another child now would duplicate the same lane rather than expose a
fresh uncovered identity/reactive capability.

## Evidence

- Active goal `d6d73d1d-9a36-4411-a8b3-508859276d79` is still
  `Identity and reactive data workflows`, with explicit requirements for Google
  login, invitations, cross-table sync, grouped aggregate maintenance, and
  batch initialize/backfill.
- Current live goal issues are only:
  - `CLO-2224` blocked, covered by `CLO-2225`.
  - `CLO-2225` in progress for this reconciliation.
- `CLO-2223` is done and closed with the same conclusion: no fresh bounded
...
- The delta maps to canonical recipe metadata for direct sync and grouped
  ...
  backfill/recompute.
- Repository search confirms coverage for Google callback invitation
...
Close `CLO-2225` as done and unblock parent `CLO-2224`. Do not create a
